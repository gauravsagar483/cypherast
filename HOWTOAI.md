...
- Match existing graph-native style: Cypher terms, zero runtime deps, typed Python 3.11+.
...
make optimize Q="..." WRITE=puppygraph CONSTRAINT_DISABLE=strip_nulls_order_modifiers
...
- Pass `schema=GraphSchema(...)` for id-field / (opt-in) undeclared-prop checks; without schema those are non-goals.
