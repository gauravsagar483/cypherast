"""Exception hierarchy and stable error-code registry for cypherglot."""
...
class CypherglotError(Exception):
...
class TokenizeError(CypherglotError):
    """Lexical analysis failure (CG10xx)."""


class ParseError(CypherglotError):
    """Syntactic analysis failure (CG11xx)."""


class ValidationError(CypherglotError):
    """Semantic validation failure (CG12xx)."""


class SchemaError(CypherglotError):
    """Schema / catalog failure (CG13xx)."""


class CompatibilityError(CypherglotError):
    """Source construct not expressible in target dialect (CG14xx)."""


class OptimizeError(CypherglotError):
    """Rewriter failure (CG15xx)."""


class PlanError(CypherglotError):
    """Planner failure (CG16xx)."""


class ExecuteError(CypherglotError):
...
class ErrorCollector:
    ...
    def __init__(self) -> None:
        self.errors: list[CypherglotError] = []

    def add(self, err: CypherglotError) -> None:
