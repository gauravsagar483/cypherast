...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="cypherast", description="Cypher/GQL toolkit")
    ...
    if args.cmd == "parse":
        tree = cypherast.parse_one(args.query, read=args.read)
        print(repr(tree))
        return 0
    if args.cmd == "translate":
        print(cypherast.translate(args.query, from_=args.read, to_=args.write, pretty=args.pretty))
        return 0
    if args.cmd == "optimize":
        tree = cypherast.optimize(args.query, read=args.read)
        print(tree.cypher(pretty=True))
        return 0
    if args.cmd == "explain":
        print(cypherast.explain(args.query))
        return 0
    if args.cmd == "run":
        from cypherast.executor import Graph

        result = cypherast.run(args.query, graph=Graph())
