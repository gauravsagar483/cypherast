...
def test_simplify_arithmetic():
    tree = cypherast.optimize("RETURN 1 + 1")
...
def test_simplify_boolean():
    tree = cypherast.optimize("RETURN true AND false")
    ret = tree.find(a.Return)
...
def test_qualify_names_anonymous():
    tree = cypherast.optimize("MATCH () RETURN 1")
...
def test_pushdown_predicate():
    tree = cypherast.optimize("MATCH (n:Person) WHERE n.age = 30 RETURN n")
    node = tree.find(a.NodePattern)
    assert node is not None
