...
def test_translate_roundtrip():
    ...
    out = cypherglot.translate(q, from_="opencypher", to_="neo4j")
    assert "MATCH" in out
    assert cypherglot.transpile(q, from_="opencypher", to_="memgraph")


def test_list_subscript_parse():
    tree = cypherglot.parse_one("RETURN split('a.b', '.')[0] AS x")
    assert tree.find(a.ListSubscript)


def test_not_pattern_predicate():
    tree = cypherglot.parse_one("MATCH (n) WHERE NOT (n)-[:R]->() RETURN n")
    assert tree.find(a.PatternPredicate)


def test_exists_pattern():
    tree = cypherglot.parse_one("MATCH (n) WHERE EXISTS ((n)-[:R]->()) RETURN n")
    assert tree.find(a.PatternPredicate)


def test_call_subquery():
    tree = cypherglot.parse_one("MATCH (n) CALL { MATCH (m) RETURN m } RETURN n")
    assert tree.find(a.CallSubquery)


def test_qpp_parse():
    tree = cypherglot.parse_one("MATCH ((a)-[:R]->(b)){1,2} RETURN a")
