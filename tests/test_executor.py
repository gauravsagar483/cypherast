...
def test_match_return():
    g = _graph()
    result = cypherast.run("MATCH (n:Person) RETURN n.name", graph=g)
...
def test_where():
    g = _graph()
    result = cypherast.run("MATCH (n:Person) WHERE n.age > 30 RETURN n.name AS name", graph=g)
...
def test_create():
    g = Graph()
    cypherast.run("CREATE (n:Person {name: 'Ada'})", graph=g)
...
def test_count():
    g = _graph()
    result = cypherast.run("MATCH (n:Person) RETURN count(n) AS c", graph=g)
    rows = list(result)
    assert rows[0]["c"] == 3
...
def test_unwind():
    g = Graph()
    result = cypherast.run("UNWIND [1, 2, 3] AS x RETURN x", graph=g)
